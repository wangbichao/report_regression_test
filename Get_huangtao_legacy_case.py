...
report_result_file = "C:\\Users\\bichao\Desktop\\Daily_report\\get_Tao_case.txt"
...
def find_false_in_sheet(sheet):
    for column in sheet.iter_cols():
        for cell in column:
            if re.search('Tao|tao', str(cell.value)):
#                print(cell.value + sheet.cell(row=cell.row, column=3).value + sheet.cell(row=cell.row, column=4).value)
                if cell.column == 'C':
                    tao_case.append(("NULL" if sheet.cell(row=cell.row, column=6).value == None else sheet.cell(row=cell.row, column=6).value)
